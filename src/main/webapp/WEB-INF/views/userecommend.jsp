        ...
        <div class="col-lg-3 col-md-4 col-sm-6 portfolio-item">
          <div class="card h-100" >
            ...
            <div class="card-body">
              ...
              <p class="card-text">추천지수: ${d.recommentRating }</p>
            </div>
          </div>
        </div>
      </c:forEach> 
	...
	<div id='moviePageNum' style="align-content: center;">
		<ul class="pagination justify-content-center" style="flex-wrap: wrap; overflow-y: auto;">
			<li class="page-item ${(0<pageNow && pageNow<11)?'disabled':''}">
				...
				<a class="page-link" href="mahoutUser.go?selectNum=${pages1*10-9}&searcheMovie=${searchMovieTitle}" aria-label="Previous" >
					<span aria-hidden="true">&laquo;</span> <span class="sr-only">Previous</span>
				</a>
			</li>
			<c:forEach var="a" begin="${pageBegin }" end="${pageEnd}" step="1">
				<li class="page-item ${pageNow == a? 'active': ''}" <%-- id="pagebutton${a}" --%> >
					<a class="page-link" href="mahoutUser.go?selectNum=${a }&searcheMovie=${searchMovieTitle}"<%-- onclick="getTop100Page(${a})" --%>>${a}</a>
				</li>
			</c:forEach>
			<li class="page-item ${(pageEndBegin<pageNow && pageNow<=pageMax)?'disabled':''}">
				...
				<a class="page-link" href="mahoutUser.go?selectNum=${pages2*10+11}&searcheMovie=${searchMovieTitle}" aria-label="Next">
